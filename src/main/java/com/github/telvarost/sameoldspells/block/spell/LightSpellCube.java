...
public class LightSpellCube extends TemplateBlock {

    public LightSpellCube(Identifier identifier, Material material) {
    ...
    public boolean isSideVisible(BlockView blockView, int x, int y, int z, int side) {
        if (  !Config.config.RENDER_SPELLS_CONFIG.DISABLE_ALL_SPELL_RENDERING
            && Config.config.RENDER_SPELLS_CONFIG.LIGHT_SPELL_CUBE
        ) {
            return true;
        } else {
            return false;
    ...
    public void onEntityCollision(World world, int x, int y, int z, Entity entity) {
        ...
        if (null != entity) {
            ...
            if (null != worldProperties) {
                ...
                if (24000L < l) {
                    worldProperties.setTime(l - (l % 24000L) + 6000L);
                } else {
                    l = l + 24000L;
                    worldProperties.setTime(l - (l % 24000L) + 6000L);
                }
