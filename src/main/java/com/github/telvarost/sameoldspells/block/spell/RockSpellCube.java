...
public class RockSpellCube extends TemplateBlock {

    public RockSpellCube(Identifier identifier, Material material) {
    ...
    public boolean isSideVisible(BlockView blockView, int x, int y, int z, int side) {
        if (  !Config.config.RENDER_SPELLS_CONFIG.DISABLE_ALL_SPELL_RENDERING
            && Config.config.RENDER_SPELLS_CONFIG.ROCK_SPELL_CUBE
        ) {
            return true;
        } else {
            return false;
