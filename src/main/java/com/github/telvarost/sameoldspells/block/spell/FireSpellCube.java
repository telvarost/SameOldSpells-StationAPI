...
public class FireSpellCube extends TemplateBlock {

    public FireSpellCube(Identifier identifier, Material material) {
    ...
    public boolean isSideVisible(BlockView blockView, int x, int y, int z, int side) {
        if (  !Config.config.RENDER_SPELLS_CONFIG.DISABLE_ALL_SPELL_RENDERING
            && Config.config.RENDER_SPELLS_CONFIG.FIRE_SPELL_CUBE
        ) {
            return true;
        } else {
            return false;
